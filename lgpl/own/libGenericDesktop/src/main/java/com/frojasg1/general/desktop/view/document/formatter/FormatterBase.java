...
public abstract class FormatterBase implements ColorThemeInvertible
{
	...
	{
		return( FrameworkComponentFunctions.instance().isDarkMode( getJTextPane() ) );
	}

	protected ColorInversor getColorInversor()
	{
		return( FrameworkComponentFunctions.instance().getColorInversor( getJTextPane() ) );
	}

	protected void setOriginalDefaultStyleColors()
	{
		if( _defaultStyleForegroundColor.get() == null )
			_defaultStyleForegroundColor.set( _pane.getForeground() );

		if( _defaultStyleBackgroundColor.get() == null )
			_defaultStyleBackgroundColor.set( _pane.getBackground() );
	}
...
*/
	...
	protected final void initialize()
	{
		...
		_invertibleColorModeColors = createOriginalInvertibleColors();
	}
