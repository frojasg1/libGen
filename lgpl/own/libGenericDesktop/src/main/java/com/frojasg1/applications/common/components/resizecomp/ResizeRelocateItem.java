											...
											SizeChangedListener
{
	...
	public static final int MASK = 2 * FILL_WHOLE_HEIGHT - 1;
	...
	protected static final int HORIZONTAL_MODIFIER_FLAGS = RESIZE_TO_RIGHT | MOVE_TO_RIGHT |
		...
		LEAVE_WIDTH_UNALTERED | FILL_WHOLE_WIDTH;
	...
	protected static final int VERTICAL_MODIFIER_FLAGS = RESIZE_TO_BOTTOM | MOVE_TO_BOTTOM |
		...
		LEAVE_HEIGHT_UNALTERED | FILL_WHOLE_HEIGHT;
...
//		return( !_parent.isResizeRelocateItemsResizeListenersBlocked() );
	...
	protected boolean isAlreadyZoomed()
	{
...
*/
		...
		if( count( isFlagActive(RESIZE_TO_RIGHT),
					isFlagActive(MOVE_TO_RIGHT),
					isFlagActive(MOVE_LEFT_SIDE_PROPORTIONAL +
		...
		if( count( isFlagActive(RESIZE_TO_BOTTOM),
					isFlagActive(MOVE_TO_BOTTOM),
					isFlagActive(MOVE_TOP_SIDE_PROPORTIONAL +
...
//		else if( isFlagActive( MOVE_TO_BOTTOM ) )
	...
	{
		_parentComponentListener = createComponentListener();
		addComponentListenerToParent( _parentComponentListener );
	...
	protected void addComponentListenerToParent( ComponentListener listener )
	{
		Component parent = getParent(_component);

		if( parent != null )
			parent.addComponentListener( _parentComponentListener );
	}
...
//		zoomInsets( zoomFactor );
	...
	protected boolean matches()
	{
		...
		if( _component != null )
			result = Objects.equals( _component.getName(), "jPanel3" );
/*
...
//					width = defaultNewBounds.width;
	...
	*/
					...
					{
						width = (int) Math.max( 
											Math.min( newBounds.getWidth() + delta, _component.getMaximumSize().getWidth() ),
	...
	//					_component.setSize( width, (int) _component.getHeight() );
				...
				else if( isFlagActive( MOVE_TO_RIGHT ) )
				{
					int delta = (int) ( parentBounds.getWidth() -
										( newBounds.getX() - parentBounds.getX() )  -
										floor( _pixelsFromTheComponentLeftBoundaryToTheRightOfTheParent, zoomFactor ) );	// the hiest x
					if( delta != 0 )
					{
						newX = (int) newBounds.getX() + delta;
	...
	//						rightBound = newX + _originalWidth;
							rightBound = newX + floor( _originalWidth, zoomFactor );	// the lowest x
						}
...
//					height = defaultNewBounds.height;
	...
	*/
					...
					{
						height = (int) Math.max(
										Math.min( newBounds.getHeight() + delta, _component.getMaximumSize().getHeight() ),
...
*/
	...
	protected Dimension calculateNewMinimumPreferredSizeForScrollableFreeComponent( Dimension newSize )
	{
		...
		if( ! Objects.equals( _scrollableFreeComponentSize, newSize ) )
		{
			...
			if( _scrollableFreeComponentSize != null )
			{
				...
				{
					Dimension parentSize = _component.getParent().getSize();
					if( isFlagActive(RESIZE_SCROLLABLE_HORIZONTAL_FREE) )
	...
	{
		Component parent = getParent( _component );
		if( ( _parentComponentListener != null ) && ( parent != null ) )
		{
