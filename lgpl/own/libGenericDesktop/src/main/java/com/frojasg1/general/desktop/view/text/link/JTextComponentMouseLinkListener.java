 ...
 *
 */
package com.frojasg1.general.desktop.view.text.link;
 ...
 */
public abstract class JTextComponentMouseLinkListener
				...
				TextUndoRedoListener
{
	...
	public JTextComponentMouseLinkListener(  )
	{
	}
...
//		_scrollByProgram = false;
	...
	protected void moveMouse( Component parent )
	{
		...
		if( parent instanceof JViewport )
		{
			...
			JTextComponentMouseLinkListener.this.changeCursor( jvp, -1 );
		}
	}

	protected void changeCursor( JViewport textComponentParent, int oldScrollYYValue )
	{
		SwingUtilities.invokeLater( () -> { changeCursor_internal( textComponentParent, oldScrollYYValue ); } );
	}
...
//			JTextComponent jtc = (JTextComponent) textComponentParent.getComponent(0);
			...
			if( ScreenFunctions.isInsideComponent( textComponentParent, pt ) )
			{
				...
				if( textComponentParent.getParent() instanceof JScrollPane )
				{
					...
					JScrollBar vSb = parent.getVerticalScrollBar();
...
//						jScrollPane1.getViewport().getComponent(0).setCursor( CursorFunctions._defaultCursor );
	...
	protected abstract boolean hasToShowHand( int pos );
	...
	public void changeCursor( int oldScrollYYValue )
	{
		JTextComponentMouseLinkListener.this.changeCursor(getJScrollPane().getViewport(), oldScrollYYValue );
	}
	...
	protected StringAndPosition getMouseText( JTextComponent tc, int pos )
	{
		...
		if( word != null )
		{
			...
			{
				_lastStringAndPosition = createStringAndPosition( initialPosition,
																finalPosition,
																word );
			}
		}
	...
	protected StringAndPosition createStringAndPosition( int initialPosition,
														int finalPosition,
														String word )
	{
		return( new StringAndPosition( initialPosition,	finalPosition, word ) );
	}
